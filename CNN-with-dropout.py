...
class MRI_CNN(nn.Module):
    def __init__(self):
        ...
        self.dropout1 = nn.Dropout(p=0.5)  # 添加 Dropout，p 为丢弃概率
        self.fc2 = nn.Linear(256, 128)
        self.dropout2 = nn.Dropout(p=0.5)  # 在第二个全连接层前添加 Dropout
    ...
    def forward(self, x):
        ...
        x = self.dropout1(x)  # Dropout after first fully connected layer
        x = torch.relu(self.fc2(x))
        x = self.dropout2(x)  # Dropout after second fully connected layer
        x = self.fc3(x)
        return x
...
train_accuracies = []
val_accuracies = []
best_val_loss = -float('inf')  # 用于存储验证损失最高的模型
